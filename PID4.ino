...
float Proporcional = 4;  //Ganancia proporcional 1
float Integral = 2;  //Ganancia integral 0.9
...
void PID(){
  ...
  if(AccionControl > 0){
    ...
    AccionControl = map(AccionControl, 0, 255, 100, 255);
    motor_off();
    CW(AccionControl);
  }
...
{ 
  ...
  if(degAngle > 0 && degAngle <= 235)
  {
    degAngle = degAngle + 125;
  }
  else if (degAngle > 235 && degAngle <= 360)
  {
    degAngle = degAngle - 235; //Angle correction
  }
