...
float Proporcional = 3;  //Ganancia proporcional 1
float Integral = 1;  //Ganancia integral 0.7
...
void PID(){
  ...
  if(AccionControl > 0){
    ...
    AccionControl = map(AccionControl, 0, 255, 120, 255);
    motor_off();
    CW(AccionControl);
  }
...
{ 
  ...
  if(degAngle > 0 && degAngle <= 340)
  {
    degAngle = degAngle + 20;
  }
  else if (degAngle > 341 && degAngle <= 360)
  {
    degAngle = degAngle - 340; //Angle correction
  }
