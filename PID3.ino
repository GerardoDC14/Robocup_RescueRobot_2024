...
void PID(){
  ...
  if(AccionControl > 0){
    ...
    AccionControl = map(AccionControl, 0, 255, 110, 255);
    motor_off();
    CW(AccionControl);
  }
...
{ 
  ...
  if(degAngle > 0 && degAngle <= 298)
  {
    degAngle = degAngle + 62;
  }
  else if (degAngle > 298 && degAngle <= 360)
  {
    degAngle = degAngle - 298; //Angle correction
  }
